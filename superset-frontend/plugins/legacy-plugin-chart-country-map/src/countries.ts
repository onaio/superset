/*
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ASF licenses this file
 * to you under the Apache License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License.  You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */

import abyei from './countries/abyei.geojson';
import afghanistan from './countries/afghanistan.geojson';
import akrotiri from './countries/akrotiri.geojson';
import aksai_chin from './countries/aksai_chin.geojson';
import aland_islands from './countries/aland_islands.geojson';
import albania from './countries/albania.geojson';
import algeria from './countries/algeria.geojson';
import american_samoa from './countries/american_samoa.geojson';
import andorra from './countries/andorra.geojson';
import angola from './countries/angola.geojson';
import anguilla from './countries/anguilla.geojson';
import antarctica from './countries/antarctica.geojson';
import antigua_and_barbuda from './countries/antigua_and_barbuda.geojson';
import argentina from './countries/argentina.geojson';
import armenia from './countries/armenia.geojson';
import aruba from './countries/aruba.geojson';
import arunachal_pradesh from './countries/arunachal_pradesh.geojson';
import ashmore_and_cartier_islands from './countries/ashmore_and_cartier_islands.geojson';
import australia from './countries/australia.geojson';
import austria from './countries/austria.geojson';
import azerbaijan from './countries/azerbaijan.geojson';
import azores_islands from './countries/azores_islands.geojson';
import bahamas from './countries/bahamas.geojson';
import bahrain from './countries/bahrain.geojson';
import baker_island from './countries/baker_island.geojson';
import bangladesh from './countries/bangladesh.geojson';
import barbados from './countries/barbados.geojson';
import bassas_da_india from './countries/bassas_da_india.geojson';
import belarus from './countries/belarus.geojson';
import belgium from './countries/belgium.geojson';
import belize from './countries/belize.geojson';
import benin from './countries/benin.geojson';
import bermuda from './countries/bermuda.geojson';
import bhutan from './countries/bhutan.geojson';
import bir_tawil from './countries/bir_tawil.geojson';
import bird_island from './countries/bird_island.geojson';
import bolivia from './countries/bolivia.geojson';
import bonaire from './countries/bonaire.geojson';
import bosnia_and_herzegovina from './countries/bosnia_and_herzegovina.geojson';
import botswana from './countries/botswana.geojson';
import bouvet_island from './countries/bouvet_island.geojson';
import brazil from './countries/brazil.geojson';
import british_virgin_islands from './countries/british_virgin_islands.geojson';
import brunei_darussalam from './countries/brunei_darussalam.geojson';
import bulgaria from './countries/bulgaria.geojson';
import burkina_faso from './countries/burkina_faso.geojson';
import burundi from './countries/burundi.geojson';
import cambodia from './countries/cambodia.geojson';
import cameroon from './countries/cameroon.geojson';
import canada from './countries/canada.geojson';
import canary_islands from './countries/canary_islands.geojson';
import cape_verde from './countries/cape_verde.geojson';
import cayman_islands from './countries/cayman_islands.geojson';
import central_african_republic from './countries/central_african_republic.geojson';
import chad from './countries/chad.geojson';
import chagos_archipelagio from './countries/chagos_archipelagio.geojson';
import chile from './countries/chile.geojson';
import china from './countries/china.geojson';
import china_india from './countries/china_india.geojson';
import christmas_island from './countries/christmas_island.geojson';
import clipperton_island from './countries/clipperton_island.geojson';
import cocos_islands from './countries/cocos_islands.geojson';
import colombia from './countries/colombia.geojson';
import comoros from './countries/comoros.geojson';
import congo from './countries/congo.geojson';
import cook_islands from './countries/cook_islands.geojson';
import costa_rica from './countries/costa_rica.geojson';
import cote_divoire from './countries/cote_divoire.geojson';
import croatia from './countries/croatia.geojson';
import cuba from './countries/cuba.geojson';
import curacao from './countries/curacao.geojson';
import cyprus from './countries/cyprus.geojson';
import czechia from './countries/czechia.geojson';
import dekelia from './countries/dekelia.geojson';
import democratic_republic_of_the_congo from './countries/democratic_republic_of_the_congo.geojson';
import denmark from './countries/denmark.geojson';
import djibouti from './countries/djibouti.geojson';
import dominica from './countries/dominica.geojson';
import dominican_republic from './countries/dominican_republic.geojson';
import ecuador from './countries/ecuador.geojson';
import egypt from './countries/egypt.geojson';
import el_salvador from './countries/el_salvador.geojson';
import equatorial_guinea from './countries/equatorial_guinea.geojson';
import eritrea from './countries/eritrea.geojson';
import estonia from './countries/estonia.geojson';
import eswatini from './countries/eswatini.geojson';
import ethiopia from './countries/ethiopia.geojson';
import europa_island from './countries/europa_island.geojson';
import falkland_islands from './countries/falkland_islands.geojson';
import faroe_islands from './countries/faroe_islands.geojson';
import fiji from './countries/fiji.geojson';
import finland from './countries/finland.geojson';
import france from './countries/france.geojson';
import french_guiana from './countries/french_guiana.geojson';
import french_polynesia from './countries/french_polynesia.geojson';
import french_southern_territories from './countries/french_southern_territories.geojson';
import gabon from './countries/gabon.geojson';
import gambia from './countries/gambia.geojson';
import gaza from './countries/gaza.geojson';
import georgia from './countries/georgia.geojson';
import germany from './countries/germany.geojson';
import ghana from './countries/ghana.geojson';
import gibraltar from './countries/gibraltar.geojson';
import glorioso_islands from './countries/glorioso_islands.geojson';
import greece from './countries/greece.geojson';
import greenland from './countries/greenland.geojson';
import grenada from './countries/grenada.geojson';
import guadeloupe from './countries/guadeloupe.geojson';
import guam from './countries/guam.geojson';
import guatemala from './countries/guatemala.geojson';
import guernsey from './countries/guernsey.geojson';
import guinea from './countries/guinea.geojson';
import guinea_bissau from './countries/guinea_bissau.geojson';
import guyana from './countries/guyana.geojson';
import haiti from './countries/haiti.geojson';
import halaib_triangle from './countries/halaib_triangle.geojson';
import heard_island_and_mcdonald_islands from './countries/heard_island_and_mcdonald_islands.geojson';
import holy_see from './countries/holy_see.geojson';
import honduras from './countries/honduras.geojson';
import hong_kong from './countries/hong_kong.geojson';
import howland_island from './countries/howland_island.geojson';
import hungary from './countries/hungary.geojson';
import iceland from './countries/iceland.geojson';
import ilemi_triangle from './countries/ilemi_triangle.geojson';
import india from './countries/india.geojson';
import indonesia from './countries/indonesia.geojson';
import iran from './countries/iran.geojson';
import iraq from './countries/iraq.geojson';
import ireland from './countries/ireland.geojson';
import isle_of_man from './countries/isle_of_man.geojson';
import israel from './countries/israel.geojson';
import italy from './countries/italy.geojson';
import jamaica from './countries/jamaica.geojson';
import jammu_and_kashmir from './countries/jammu_and_kashmir.geojson';
import japan from './countries/japan.geojson';
import jarvis_island from './countries/jarvis_island.geojson';
import jersey from './countries/jersey.geojson';
import johnston_atoll from './countries/johnston_atoll.geojson';
import jordan from './countries/jordan.geojson';
import juan_de_nova_island from './countries/juan_de_nova_island.geojson';
import kazakhstan from './countries/kazakhstan.geojson';
import kenya from './countries/kenya.geojson';
import kingman_reef from './countries/kingman_reef.geojson';
import kiribati from './countries/kiribati.geojson';
import korea from './countries/korea.geojson';
import kosovo from './countries/kosovo.geojson';
import kuril_islands from './countries/kuril_islands.geojson';
import kuwait from './countries/kuwait.geojson';
import kyrgyzstan from './countries/kyrgyzstan.geojson';
import lao from './countries/lao.geojson';
import latvia from './countries/latvia.geojson';
import lebanon from './countries/lebanon.geojson';
import lesotho from './countries/lesotho.geojson';
import liberia from './countries/liberia.geojson';
import libya from './countries/libya.geojson';
import liechtenstein from './countries/liechtenstein.geojson';
import lithuania from './countries/lithuania.geojson';
import luxembourg from './countries/luxembourg.geojson';
import macao from './countries/macao.geojson';
import madagascar from './countries/madagascar.geojson';
import madeira_islands from './countries/madeira_islands.geojson';
import malawi from './countries/malawi.geojson';
import malaysia from './countries/malaysia.geojson';
import maldives from './countries/maldives.geojson';
import mali from './countries/mali.geojson';
import malta from './countries/malta.geojson';
import marshall_islands from './countries/marshall_islands.geojson';
import martinique from './countries/martinique.geojson';
import mauritania from './countries/mauritania.geojson';
import mauritius from './countries/mauritius.geojson';
import mayotte from './countries/mayotte.geojson';
import mexico from './countries/mexico.geojson';
import micronesia from './countries/micronesia.geojson';
import midway_islands from './countries/midway_islands.geojson';
import moldova from './countries/moldova.geojson';
import monaco from './countries/monaco.geojson';
import mongolia from './countries/mongolia.geojson';
import montenegro from './countries/montenegro.geojson';
import montserrat from './countries/montserrat.geojson';
import morocco from './countries/morocco.geojson';
import mozambique from './countries/mozambique.geojson';
import myanmar from './countries/myanmar.geojson';
import namibia from './countries/namibia.geojson';
import nauru from './countries/nauru.geojson';
import navassa_island from './countries/navassa_island.geojson';
import nepal from './countries/nepal.geojson';
import netherlands from './countries/netherlands.geojson';
import new_caledonia from './countries/new_caledonia.geojson';
import new_zealand from './countries/new_zealand.geojson';
import nicaragua from './countries/nicaragua.geojson';
import niger from './countries/niger.geojson';
import nigeria from './countries/nigeria.geojson';
import niue from './countries/niue.geojson';
import norfolk_island from './countries/norfolk_island.geojson';
import north_macedonia from './countries/north_macedonia.geojson';
import northern_mariana_is from './countries/northern_mariana_is.geojson';
import norway from './countries/norway.geojson';
import oman from './countries/oman.geojson';
import pakistan from './countries/pakistan.geojson';
import palau from './countries/palau.geojson';
import palmyra_atoll from './countries/palmyra_atoll.geojson';
import panama from './countries/panama.geojson';
import papua_new_guinea from './countries/papua_new_guinea.geojson';
import paracel_islands from './countries/paracel_islands.geojson';
import paraguay from './countries/paraguay.geojson';
import peru from './countries/peru.geojson';
import philippines from './countries/philippines.geojson';
import pitcairn from './countries/pitcairn.geojson';
import poland from './countries/poland.geojson';
import portugal from './countries/portugal.geojson';
import puerto_rico from './countries/puerto_rico.geojson';
import qatar from './countries/qatar.geojson';
import republic_of_korea from './countries/republic_of_korea.geojson';
import republic_of_turkiye from './countries/republic_of_turkiye.geojson';
import reunion from './countries/reunion.geojson';
import romania from './countries/romania.geojson';
import russian_federation from './countries/russian_federation.geojson';
import rwanda from './countries/rwanda.geojson';
import saba from './countries/saba.geojson';
import saint_barthelemy from './countries/saint_barthelemy.geojson';
import saint_helena from './countries/saint_helena.geojson';
import saint_helena_ascencion from './countries/saint_helena_ascencion.geojson';
import saint_helena_gough from './countries/saint_helena_gough.geojson';
import saint_helena_tristan_da_cunha from './countries/saint_helena_tristan_da_cunha.geojson';
import saint_kitts_and_nevis from './countries/saint_kitts_and_nevis.geojson';
import saint_lucia from './countries/saint_lucia.geojson';
import saint_martin from './countries/saint_martin.geojson';
import saint_pierre_et_miquelon from './countries/saint_pierre_et_miquelon.geojson';
import saint_vincent_and_the_grenadines from './countries/saint_vincent_and_the_grenadines.geojson';
import samoa from './countries/samoa.geojson';
import san_marino from './countries/san_marino.geojson';
import sao_tome_and_principe from './countries/sao_tome_and_principe.geojson';
import saudi_arabia from './countries/saudi_arabia.geojson';
import scarborough_reef from './countries/scarborough_reef.geojson';
import senegal from './countries/senegal.geojson';
import senkaku_islands from './countries/senkaku_islands.geojson';
import serbia from './countries/serbia.geojson';
import seychelles from './countries/seychelles.geojson';
import sierra_leone from './countries/sierra_leone.geojson';
import singapore from './countries/singapore.geojson';
import sint_eustatius from './countries/sint_eustatius.geojson';
import sint_maarten from './countries/sint_maarten.geojson';
import slovakia from './countries/slovakia.geojson';
import slovenia from './countries/slovenia.geojson';
import solomon_islands from './countries/solomon_islands.geojson';
import somalia from './countries/somalia.geojson';
import south_africa from './countries/south_africa.geojson';
import south_georgia_and_the_south_sandwich_islands from './countries/south_georgia_and_the_south_sandwich_islands.geojson';
import south_sudan from './countries/south_sudan.geojson';
import sovereignty_unsettled from './countries/sovereignty_unsettled.geojson';
import spain from './countries/spain.geojson';
import spratly_islands from './countries/spratly_islands.geojson';
import sri_lanka from './countries/sri_lanka.geojson';
import state_of_palestine from './countries/state_of_palestine.geojson';
import sudan from './countries/sudan.geojson';
import suriname from './countries/suriname.geojson';
import svalbard_and_jan_mayen_islands from './countries/svalbard_and_jan_mayen_islands.geojson';
import sweden from './countries/sweden.geojson';
import switzerland from './countries/switzerland.geojson';
import syrian_arab_republic from './countries/syrian_arab_republic.geojson';
import taiwan from './countries/taiwan.geojson';
import tajikistan from './countries/tajikistan.geojson';
import thailand from './countries/thailand.geojson';
import timor_leste from './countries/timor_leste.geojson';
import togo from './countries/togo.geojson';
import tokelau from './countries/tokelau.geojson';
import tonga from './countries/tonga.geojson';
import trinidad_and_tobago from './countries/trinidad_and_tobago.geojson';
import tromelin_island from './countries/tromelin_island.geojson';
import tunisia from './countries/tunisia.geojson';
import turkmenistan from './countries/turkmenistan.geojson';
import turks_and_caicos_islands from './countries/turks_and_caicos_islands.geojson';
import tuvalu from './countries/tuvalu.geojson';
import uganda from './countries/uganda.geojson';
import uk from './countries/uk.geojson';
import ukraine from './countries/ukraine.geojson';
import united_arab_emirates from './countries/united_arab_emirates.geojson';
import united_republic_of_tanzania from './countries/united_republic_of_tanzania.geojson';
import united_states_of_america from './countries/united_states_of_america.geojson';
import united_states_virgin_islands from './countries/united_states_virgin_islands.geojson';
import uruguay from './countries/uruguay.geojson';
import uzbekistan from './countries/uzbekistan.geojson';
import vanuatu from './countries/vanuatu.geojson';
import venezuela from './countries/venezuela.geojson';
import viet_nam from './countries/viet_nam.geojson';
import wake_island from './countries/wake_island.geojson';
import wallis_and_futuna from './countries/wallis_and_futuna.geojson';
import west_bank from './countries/west_bank.geojson';
import western_sahara from './countries/western_sahara.geojson';
import yemen from './countries/yemen.geojson';
import zambia from './countries/zambia.geojson';
import zimbabwe from './countries/zimbabwe.geojson';

export const countries = {
  abyei,
  afghanistan,
  akrotiri,
  aksai_chin,
  aland_islands,
  albania,
  algeria,
  american_samoa,
  andorra,
  angola,
  anguilla,
  antarctica,
  antigua_and_barbuda,
  argentina,
  armenia,
  aruba,
  arunachal_pradesh,
  ashmore_and_cartier_islands,
  australia,
  austria,
  azerbaijan,
  azores_islands,
  bahamas,
  bahrain,
  baker_island,
  bangladesh,
  barbados,
  bassas_da_india,
  belarus,
  belgium,
  belize,
  benin,
  bermuda,
  bhutan,
  bir_tawil,
  bird_island,
  bolivia,
  bonaire,
  bosnia_and_herzegovina,
  botswana,
  bouvet_island,
  brazil,
  british_virgin_islands,
  brunei_darussalam,
  bulgaria,
  burkina_faso,
  burundi,
  cambodia,
  cameroon,
  canada,
  canary_islands,
  cape_verde,
  cayman_islands,
  central_african_republic,
  chad,
  chagos_archipelagio,
  chile,
  china,
  china_india,
  christmas_island,
  clipperton_island,
  cocos_islands,
  colombia,
  comoros,
  congo,
  cook_islands,
  costa_rica,
  cote_divoire,
  croatia,
  cuba,
  curacao,
  cyprus,
  czechia,
  dekelia,
  democratic_republic_of_the_congo,
  denmark,
  djibouti,
  dominica,
  dominican_republic,
  ecuador,
  egypt,
  el_salvador,
  equatorial_guinea,
  eritrea,
  estonia,
  eswatini,
  ethiopia,
  europa_island,
  falkland_islands,
  faroe_islands,
  fiji,
  finland,
  france,
  french_guiana,
  french_polynesia,
  french_southern_territories,
  gabon,
  gambia,
  gaza,
  georgia,
  germany,
  ghana,
  gibraltar,
  glorioso_islands,
  greece,
  greenland,
  grenada,
  guadeloupe,
  guam,
  guatemala,
  guernsey,
  guinea,
  guinea_bissau,
  guyana,
  haiti,
  halaib_triangle,
  heard_island_and_mcdonald_islands,
  holy_see,
  honduras,
  hong_kong,
  howland_island,
  hungary,
  iceland,
  ilemi_triangle,
  india,
  indonesia,
  iran,
  iraq,
  ireland,
  isle_of_man,
  israel,
  italy,
  jamaica,
  jammu_and_kashmir,
  japan,
  jarvis_island,
  jersey,
  johnston_atoll,
  jordan,
  juan_de_nova_island,
  kazakhstan,
  kenya,
  kingman_reef,
  kiribati,
  korea,
  kosovo,
  kuril_islands,
  kuwait,
  kyrgyzstan,
  lao,
  latvia,
  lebanon,
  lesotho,
  liberia,
  libya,
  liechtenstein,
  lithuania,
  luxembourg,
  macao,
  madagascar,
  madeira_islands,
  malawi,
  malaysia,
  maldives,
  mali,
  malta,
  marshall_islands,
  martinique,
  mauritania,
  mauritius,
  mayotte,
  mexico,
  micronesia,
  midway_islands,
  moldova,
  monaco,
  mongolia,
  montenegro,
  montserrat,
  morocco,
  mozambique,
  myanmar,
  namibia,
  nauru,
  navassa_island,
  nepal,
  netherlands,
  new_caledonia,
  new_zealand,
  nicaragua,
  niger,
  nigeria,
  niue,
  norfolk_island,
  north_macedonia,
  northern_mariana_is,
  norway,
  oman,
  pakistan,
  palau,
  palmyra_atoll,
  panama,
  papua_new_guinea,
  paracel_islands,
  paraguay,
  peru,
  philippines,
  pitcairn,
  poland,
  portugal,
  puerto_rico,
  qatar,
  republic_of_korea,
  republic_of_turkiye,
  reunion,
  romania,
  russian_federation,
  rwanda,
  saba,
  saint_barthelemy,
  saint_helena,
  saint_helena_ascencion,
  saint_helena_gough,
  saint_helena_tristan_da_cunha,
  saint_kitts_and_nevis,
  saint_lucia,
  saint_martin,
  saint_pierre_et_miquelon,
  saint_vincent_and_the_grenadines,
  samoa,
  san_marino,
  sao_tome_and_principe,
  saudi_arabia,
  scarborough_reef,
  senegal,
  senkaku_islands,
  serbia,
  seychelles,
  sierra_leone,
  singapore,
  sint_eustatius,
  sint_maarten,
  slovakia,
  slovenia,
  solomon_islands,
  somalia,
  south_africa,
  south_georgia_and_the_south_sandwich_islands,
  south_sudan,
  sovereignty_unsettled,
  spain,
  spratly_islands,
  sri_lanka,
  state_of_palestine,
  sudan,
  suriname,
  svalbard_and_jan_mayen_islands,
  sweden,
  switzerland,
  syrian_arab_republic,
  taiwan,
  tajikistan,
  thailand,
  timor_leste,
  togo,
  tokelau,
  tonga,
  trinidad_and_tobago,
  tromelin_island,
  tunisia,
  turkmenistan,
  turks_and_caicos_islands,
  tuvalu,
  uganda,
  uk,
  ukraine,
  united_arab_emirates,
  united_republic_of_tanzania,
  united_states_of_america,
  united_states_virgin_islands,
  uruguay,
  uzbekistan,
  vanuatu,
  venezuela,
  viet_nam,
  wake_island,
  wallis_and_futuna,
  west_bank,
  western_sahara,
  yemen,
  zambia,
  zimbabwe,
};

export const countryOptions = Object.keys(countries).map(x => {
  if (x === 'uk' || x === 'usa') {
    return [x, x.toUpperCase()];
  }
  if (x === 'italy_regions') {
    return [x, 'Italy (regions)'];
  }
  return [x, x[0].toUpperCase() + x.slice(1)];
});

export default countries;
